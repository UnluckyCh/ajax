function addEditDeleteButtons(task_elem) {
            var editBtn = $('<span>')
                ...
                .text('✎')
                .on('click', function() {
                    var currentTitle = task_elem.find('span.task-text').text();
                    var currentBody = task_elem.find('div.task-body').text();
                    var updatedTask = prompt('Редактировать задание:', currentTitle + '\n\n' + currentBody);

                    if (updatedTask !== null) {
                        var updatedTaskArray = updatedTask.split('\n\n');
                        var updatedTitle = updatedTaskArray[0];
                        var updatedBody = updatedTaskArray[1] || '';
                        task_elem.find('span.task-text').text(updatedTitle);
                        task_elem.find('div.task-body').text(updatedBody);
                    }
                });
